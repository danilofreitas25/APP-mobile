...
export class HomePage implements OnInit {
  ...
  ngOnInit(){
    ...
    //this.carregando();
    this.utility.carrengando('Carregando...', 1000);
 ...
 /* async toastando(mensagem: string, posicao: "top" | "middle" | "bottom", colorir: string){
  ...
  async alertando(){
        ...
        //BOTÃO DE CADASTRAR
        {
          ...
          handler: (form:any) => {
            ...
            let item = {
              ...
              //VAI SER A VARIÁVEL DE CONTROLE NO NG IF
   ...
   async checando(product: Produtos){
    const actionSheet = await this.actionSheetCtrl.create({
      header: 'O que deseja fazer?',
            
      buttons: [
        {
          text: product.status? 'Desmarcar Produto': 'Marcar Produto',
          icon: product.status? 'radio-button-off' : 'checkmark-circle',

          handler: ()=>{
            console.log(product.status);
            product.status = !product.status;
            this.banco.statusItem(product);
